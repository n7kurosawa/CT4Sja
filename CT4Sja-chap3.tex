...
$S$を集合とし$R\ss S\times S$を$S$上の二項関係とする. $(s,s')\in R$であるとき$(s,s')\in R$と書くことにしよう. このとき全ての$s,s',s''\in S$に対し以下がみたされるならば$R$は\emph{前順序(preorder)}\index{order!preorder}と呼ばれる.
\begin{description}
...
\end{description}
半順序かつ全ての$s,s'\in S$に対して以下がみたされるとき, $R$は\emph{線形順序(linear order)}\index{order!linear order}と呼ばれる.
\begin{description}
\item[全順序性(Comparability):] $s\leq s'$あるいは$s'\leq s$のどちらかである.
\end{description}
