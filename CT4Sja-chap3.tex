...
これらを併せたもの, すなわちExample \ref{ex:department store 1} における言明 E.1, E.2, E.3, E.4, D.1, D.2, およびRule 1とRule 2は, 我々がデータベースの\emph{スキーマ(schema)}と呼ぶことになるところのものを構成する. \index{schema!of a database}\index{database!schema} 我々はSection \ref{sec:schemas}でこのアイデアを形式化する.
...
経路同値宣言(PED)の任意の集合は合同関係を生成する. 我々は合同関係(の集合)とそれを生成するPEDの集合との間の区別をしばしば省略することになる. 
...
\begin{example}[離散力学系]\label{ex:dds}\index{dynamical system!discrete}
...
\begin{example}[有限の階層]\label{ex:finite hierarchy}\index{hierarchy}
...
\subsubsection{ologとスキーマ}\label{sec:olog as db schema}\index{olog!as database schema}
...
データベーススキーマは見掛けを変えたologに他ならないことは, もう明かであろう. 違いは基本的にはologに対しては英語として読めることが求められているという点である. この章ではologの概念に重要な事項を加える, すなわちologにはデータを記入することができるという点である. 逆に言えば, データベースを理解することはologを理解することと同程度にしか難しくはないことを見る.
...
以下のologを考える.
\begin{align}\label{dia:moon1}\obox{}{.5in}{a moon}\Too{\tn{orbits}}\obox{}{.5in}{a planet}\end{align}
我々はこの関係のいくつかの事例を以下のテーブルを用いて文章化できる.
\begin{align}\label{dia:moon2}
\begin{tabular}{| c || c |}\bhline
\multicolumn{2}{| c |}{\textbf{orbits}}\\\bhline
\textbf{a moon}& \textbf{a planet}\\\bbhline
The Moon&Earth\\\hline 
Phobos&Mars\\\hline 
Deimos&Mars\\\hline 
Ganymede & Jupiter\\\hline
...
明らかに, 衛星がolog作者によって発見されるたびに(それが望遠鏡によってであれ, 会話によってであれ, 研究によってであれ), この事例のテーブルは更新することができる.
...
実際には, Example \ref{ex:orbits}はRule \ref{rules:schema to tables}に従っていない. これらの規則を厳密に守り, \eqref{dia:moon2}のデータをスキーマ\eqref{dia:moon1}に合致するテーブルへと書き写せ.
\end{exercise}
...
%\endsexc
\sexc 箱\fakebox{a thing I own}, \fakebox{a place}, および追加で一本の矢を用いた, ``無作為に''ものを置く場所を決めることの想起を助けるであろうスキーマを書き下せ.
\next この矢に対するよい見出しは何になるだろうか?
\next 対応するテーブルの集合の行をいくつか, 読者自身の事例で埋めよ.
\endsexc
\end{exercise}
...
\xymatrix{\obox{C}{.4in}{a child}\LA{rr}{has}&&\obox{F}{.5in}{a father}\ar@/^1pc/[ll]^{\tn{has as first}}\ar@/_1pc/[ll]_{\tn{has as tallest}}}
$$
\sexc このologに対してどのような経路同値宣言が適切だろうか? よいと思うならば$f\taking F\to C$, $t\taking F\to C$, および$h\taking C\to F$を用いてもよい. 
\next 合同関係のなかにはいくつPEDがあるだろうか?
\endsexc
\end{exercise}
...
データベーススキーマ$(G,\simeq)$が与えられたとき, その事例とは特定の配置に合致するデータを持つ, テーブル群である. これらはこれまでの節を通じて見ることができる. 具体的にはスキーマ\eqref{dia:maincat schema}とテーブル\eqref{dia:instance on maincat}およびテーブル\eqref{dia:instance on maincat 2}の間の関係, またスキーマ\eqref{dia:loop}と\eqref{dia:dds data}の間の関係である. 以下はその数学的な定義である.
...
$\mcC=(G,\simeq)$で$G=(V,A,\src,\tgt)$とする. \emph{$\mcC$の事例(instance on $\mcC$)}は$(\PK,\FK)\taking\mcC\to\Set$と表記され, 以下のように定義される.  構成物(A. 主キー部分, B. 外部キー部分)を告知し, それらが法則(1. 合同関係の保存)に従うことを表明する. 具体的には, 
\begin{enumerate}[\hsp A.]
\item 函数$\PK\taking V\to \Set$; i.e. それぞれの頂点$v\in V$に対して集合$\PK(v)$を与える.\footnote{$\PK(v)$の要素はテーブル$v$の行, あるいはより正確にはこれらの行の最左の升目として記載される.}
\item 各々の$v=\src(a)$かつ$w=\tgt(a)$なる矢$a\in A$に対して函数$\FK(a)\taking\PK(v)\to\PK(w)$.
\footnote{矢$a$は列に対応することになる. そしてそれぞれの行$r\in\PK(v)$に対して升目$(r,a)$はデータ$\FK(a)(r)$を含む.}
\end{enumerate}
を告知し, 任意の頂点$v, w$と$v$から$w$への経路$p=va_1a_2\ldots a_m$および$q=va_1'a_2'\ldots a_n'$に対して以下の法則が成立することを表明する.
\begin{enumerate}[\hsp 1.]
\item $p\simeq q$ならば, 全ての$x\in\PK(v)$に対して, $\PK(w)$では$$\FK(a_m)\circ\cdots\circ\FK(a_2)\circ\FK(a_1)(x)=\FK(a_n')\circ\cdots\circ\FK(a_2')\circ\FK(a_1')(x)$$が得られる.
\end{enumerate}
...
\begin{align}\label{dia:self email}
\begin{tabular}{| l || l |}\bhline
\multicolumn{2}{| c |}{\texttt{a self-email}}\\\bhline
\textbf{ID}&\textbf{is}\\\bbhline 
SEm1207&Em1207\\\hline 
SEm1210&Em1210\\\hline 
SEm1211&Em1211\\\bhline
\end{tabular}&\hsp
\begin{tabular}{| l || l | l |}\bhline
\multicolumn{3}{| c |}{\texttt{an email}}\\\bhline 
\textbf{ID}&\textbf{is sent by}&\textbf{is sent to}\\\bbhline 
Em1206&Bob&Sue\\\hline 
Em1207 &Carl&Carl\\\hline 
Em1208&Sue & Martha\\\hline 
Em1209&Chris&Bob\\\hline 
...
\end{tabular}\hsp
\begin{tabular}{| l ||}\bhline
\multicolumn{1}{| c |}{\texttt{a person}}\\\bhline 
\textbf{ID}\\\bbhline 
Bob\\\hline 
Carl\\\hline 
Chris\\\hline 
...
%\endsexc
\sexc 集合$\PK(\fakebox{an email})$は何か? 
\next 集合$\PK(\fakebox{a person})$は何か? 
\next 函数$\FK(\tn{is sent by})\taking\PK(\fakebox{an email})\to\PK(\fakebox{a person})$は何か?
\next $\mcC$の下の文書を英語化した単純な経路同値宣言として解釈せよ. 事例\eqref{dia:self email}はこれを満たす. これはすなわち, Definition \ref{def:instance}の法則1を満たすということだろうか?\index{Englishifiication}
\endsexc
\end{exercise}

%\begin{example}[Monoid action table]\label{ex:monoid action table}
\begin{example}[モノイド作用表]\label{ex:monoid action table}
...
我々はExample \ref{ex:monoid as olog}で, どのようにすればモノイド$\mcM$を対象が一つしかないologとして把握できるかを見た. データベーススキーマとして見た場合, このことはテーブルが一つだけ存在していることを意味している. 各々の$\mcM$の生成子はテーブルの列になる. このようなスキーマのデータベース事例の概念は, Section \ref{sec:monoid action table}での作用表の概念に正確に対応している. モノイドはそれ自身に作用することができ, この場合には作用表はExample \ref{ex:multiplication table}でのようなモノイドの積の表になるが, しかし一方でExample \ref{ex:action table}のように任意の他の集合に作用できることにも注意しよう. $\mcM$が集合$S$に作用するならば, 作用表の行の集合は$S$になる.

\end{example}

\begin{exercise}
%Draw (as a graph) the schema for which Table \ref{dia:action table for FSM} is an instance.
Table \ref{dia:action table for FSM}がその事例となるようなスキーマを(グラフとして)描け.
\end{exercise}
...
$\mcM$はモノイドで, そのいくつかの事例がテーブルの形で書き下されているとする. $\mcM$が群でありうる. $\mcM$の事例のテーブルにおいて, $\mcM$が群であることを示唆する証拠はどのようなものがあるだろうか? 
\end{exercise}

%% Subsubsection %%

%\subsubsection{Paths through a database}
\subsubsection{データベースを通る経路}
...
$\mcC:=(G,\simeq)$をスキーマとし$(\PK,\FK)\taking\mcC\to\Set$を$\mcC$の事例とする. このとき$G$の各々の矢$a\taking v\to w$に対して函数$\FK(a)\taking\PK(v)\to\PK(w)$が得られる. 函数は合成可能であり, よって実際には$G$を通る全ての経路に対して函数が得られる. すなわち, $p=v_0a_1,a_2,\ldots,a_n$が$v_0$から$v_n$への経路であるならば, その事例によって函数$$\FK(p):=\FK(a_n)\circ\cdots\FK(a_2)\circ\FK(a_1)\taking\PK(v_0)\to\PK(v_n)$$が得られ, これはまずはDefinition \ref{def:instance}の法則1としての外見を持つ.
...
Example \ref{ex:department store 3}の百貨店のスキーマと, \eqref{dia:maincat schema}の\texttt{Employee}から\texttt{LastNameString}への経路$[\tn{worksIn, secretary, last}]$を考える. この事例では経路は従業員の集合から名字の集合への函数として解釈できる. これは身につけておくと便利な函数になりうるだろう. (\ref{dia:instance on maincat})の事例では以下の函数が導かれる.
...
\begin{align*}
\begin{tabular}{| l || l |}\bhline
\multicolumn{2}{| c |}{\texttt{Employee}}\\\bhline 
\textbf{ID}&\textbf{Secr. name}\\\bbhline 
101&Hilbert\\\hline 
102&Russell\\\hline 
103&Hilbert\\\bhline
\end{tabular}
...
\eqref{dia:loop}の$\Loop$スキーマ上の経路$p:=[f,f]$を考える. \eqref{dia:dds data}の事例($\PK(s)=\{A,B,C,D,E,F,G,H\}$である)を用いて, $p$を函数$\PK(s)\to\PK(s)$と解釈し, 上記Example \ref{ex:paths as functions}でのような2列のテーブルとしてこれを書け.
\end{exercise}
...
%\endsexc
\sexc スキーマ$\mcC$上の事例$(\PK,\FK)$と自明な経路$p$ (i.e. $p$の長さは0. $p$はある頂点から出発するがどこへも行かない.)が与えられたとき, $p$から得られる函数はなにか?
\next $p$の始域と終域はなにか?
\endsexc
\end{exercise}
