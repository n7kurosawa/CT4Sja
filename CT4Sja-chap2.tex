...
$A$と$B$を可観測量とし, $R$をこれら二つの変数を対象として実行される実験とする. 例えば$T$を, 固定サイズの容器のなかで, \href{http://en.wikipedia.org/wiki/Ideal_gas_law}{気体}がとれる温度とし, $P$をその気体がとれる圧力とする. 1000回温度を変えて実験を実行し, 同時に圧力を記録する. これはスパン$T\From{f}E\To{g}P$である. 結果は以下のようになるだろう\footnote{訳注:\label{translator footnote dimension and uncertainty} 自然科学では, 一般に数値は次元(dimension)をともなう. また一般に測定された数値は連続量である限り不確かさ(uncertanity)を必ずともなう. しかし本書は化学や物理学の教科書ではなく数学の教科書であるため, 具体的な数値の扱いに関しては大らかに見逃すことにする.\index{a warning!oversimplified science}}.
$$
\begin{tabular}{| l || l | l |}
\bhline
...
任意の集合$B$に対して\emph{べき集合$B$ (power set of $B$)}\index{power set}を, $B$の部分集合の集合として定義し, $\PP(B)$\index{a symbol!$\PP$}で示す.
...
と呼ばれる.\index{simplex} 任意の部分集合$u\ss V$は濃度$|u|$を持ち, よってそれぞれの単体からその濃度への函数$X\to\NN$が得られる. 濃度$n+1$の単体の集合は$X_n$と表記され, それぞれの要素$x\in X_n$は\emph{$n$単体($n$-simplex)}と呼ばれる.
...
$\Set$に対する\emph{部分対象分類子(subobject classifier)}を$\Omega$\index{a symbol!$\Omega$}と表記し, 集合$\Omega:=\{\mathit{True},\mathit{False}\}$で一要素集合の要素を$\mathit{True}$へ送る函数$\singleton\to\Omega$を伴うものと定義する. 
...
\begin{definition}[特性函数]\index{characteristic function}\index{subset!characteristic function of}
...
$B$を任意の集合とし$\PP(B)$をそのべき集合とする. Proposition \ref{prop:characteristic function}によると$\PP(B)$と$\Omega^B$の間には全単射が存在する. $\Omega$の濃度は2だから, $\PP(B)$の濃度は$2^{|B|}$で, これはExercise \ref{exc:size of power sets}の正しい答えを説明している.
...
\begin{definition}[モノ射, エピ射]\label{def:mono, epi in set}\index{epimorphism!in $\Set$}\index{monomorphism!in $\Set$}
...
\subsubsection{多重集合}\index{multiset}
...
\subsubsection{添字付けられた集合}\label{sec:indexed sets}\index{indexed set}
