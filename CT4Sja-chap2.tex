...
X\ar[r]_f&Z}
\end{align}
は可換であることに注意せよ. Diagram \ref{dia:fp sets}の設定が与えられたとき, \emph{$Z$上の$X$と$Y$の引き戻し(pullback of $X$ and $Y$ over $Z$)}を, 同型写像$W\To{\iso}X\times_ZY$を得ることができるところの任意の写像$W$として定義する. Diagram \ref{dia:pullback sets}での角の記号$\lrcorner$は$W$が引き戻しであることを示す.\index{a symbol!$\lrcorner$}
