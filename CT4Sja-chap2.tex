...
$X$と$Y$を集合とする. \emph{$X$と$Y$の積(product of $X$ and $Y$)}を, 順序付きの対の集合$(x,y)$と定義し, $X\times Y$\index{a symbol!$\times$}と書く. ここで$x\in X$および$y\in Y$である. 記号的に書くと$$X\times Y=\{(x,y)\|x\in X,\;\; y\in Y\}$$である. 二つの自然な\emph{射影函数(projection functions)} $\pi_1\taking X\times Y\to X$と$\pi_2\taking X\times Y\to Y$が存在する. \index{projection functions}\index{product!projection functions}
$$\xymatrix@=15pt{&X\times Y\ar[ddr]^{\pi_2}\ar[ddl]_{\pi_1}\\\\X&&Y}$$
